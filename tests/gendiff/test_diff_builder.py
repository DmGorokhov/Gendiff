...
def test_generate_diff():
    ...
    diff_default_nested = generate_diff(filepath_old_nested,
                                        filepath_new_nested)

    assert diff_default_plain == expected_stylish_plain
    assert diff_default_nested == expected_stylish_nested

    diff_stylish_plain = generate_diff(filepath_old_plain,
                                       filepath_new_plain,
                                       'stylish')
    diff_stylish_nested = generate_diff(filepath_old_nested,
                                        filepath_new_nested,
                                        'stylish')
    ...
    diff_plain = generate_diff(filepath_old_nested,
                               filepath_new_nested,
                               'plain')

    assert diff_plain == expected_plain_output

    diff_json_plain = json.loads(generate_diff(filepath_old_plain,
                                               filepath_new_plain,
                                               'json'))
    diff_json_nested = json.loads(generate_diff(filepath_old_nested,
                                                filepath_new_nested,
                                                'json'))
